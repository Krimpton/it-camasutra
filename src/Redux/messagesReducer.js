...
const messagesReducer = (state = initialState, action) => {
    switch (action.type) {
        case 'ADD_MESSAGE':
            ...
            state.newMessageText = ''; // Зануление textArea
            state.messageInfo.push({id: 8, sms: newMessage}); // Пушит данные из textArea в конец масива из state
            return state;
        }
        case 'UPDATE_NEW_MESSAGE_TEXT':
            state.newMessageText = action.newMessage; // сообщает внешнему миру об изменении state
            return state;
